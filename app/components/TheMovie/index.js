        ...
        >
          ...
          >
            ...
            <div>
              ...
              <Icon name="line chart" />{`\xa0\xa0\xa0\xa0\xa0\xa0${movie.vote_average}\xa0\xa0\xa0\xa0\xa0\xa0${movie.vote_count} votes `}
            </Button>
            </div>
            <Overlay title={movie} />
          ...
          <Grid.Column mobile={16} computer={12} style={{ padding: '0.5rem' }}>
            ...
            <div>{movie.genres.map((item) => (<span style={{margin:'0 .6em .5em 0'}} className='movie-tags pt-tag pt-intent-success pt-minimal'>{item.name}</span>))}</div>
            {movie.overview}
            ...
            <div >
              ...
              {movie.credits.crew.map(item => {
                if (item.job === 'Screenplay') return ( <Link to={`/name/${item.id}`}>{`${item.name} `}</Link>);
              })}
            </div>
            ...
            <div className="credits">
              ...
              {movie.credits.cast.slice(0, 19).map(item => (
                <Link to={`/name/${item.id}`}>
                  <Label size='large' style={{ fontWeight:'600',margin: '3' }} as="a" image >
