...
class Header extends React.Component { // eslint-disable-line react/prefer-stateless-function
  ...
  resetComponent = () => this.setState({ isLoading2: false, value2: '', results2: [] })
  ...
  handleSearchChange= (e) => {
    ...
    setTimeout(() => {
      if (this.state.value2.length > 0) {
        this.props.search();
      } else if (this.state.value2.length < 1 || this.props.searchResults.length === 0) return this.resetComponent();
    }, 20);
    setTimeout(() => {
      const divi = <div></div>;
      const results = this.props.searchResults.results.map((item, id) => ({
        key: id,
        title: item.original_title || item.name,
        image: item.poster_path ? `https://image.tmdb.org/t/p/w185/${item.poster_path || item.profile_path}` : <Noposter />,
    ...
    return (
      <div>
        <StyledMenu style={{ justifyContent: 'center' }}>
          <Menu.Item header><Link style={{color:'#474747'}} activeStyle={{ color: '#000' }} to='/'>MovieSpot</Link></Menu.Item>
          <Menu.Item as='a'>
            <Link style={{color:'#333'}} activeStyle={{ color: '#000' }} to='/movie/334543'>
            Watchlist
            <Label basic style={{ marginLeft: '.5em' }} >649</Label>
            </Link>
          </Menu.Item>
          ...
          <Menu.Item href="https://github.com/varxyz" target="_blank" as='a'>
            <A ><Icon style={{ marginBottom: '0.2em' }} color="grey" size="large" name="github" /></A>
          </Menu.Item>
          <Menu.Item >
            <Button  basic size="mini">Log-in</Button>
          </Menu.Item>
        </StyledMenu>
      </div>
    );
